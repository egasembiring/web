...
if (isset($_POST['edit_layanan'])) {
	...
	$post_harga = $db->real_escape_string(e(@$_POST['harga']));
        ...
        $post_harga_premium = $db->real_escape_string(e(@$_POST['harga_premium']));
        $post_harga_h2h = $db->real_escape_string(e(@$_POST['harga_h2h']));
	...
	if (!$post_pid) {
		$_SESSION['alert'] = ['danger', 'Failed!', 'Masih ada form kosong.'];
	        ...
	        $edit_layanan = $db->query("UPDATE layanan_pulsa SET layanan = '{$post_layanan}', note = '{$post_note}', harga = '{$post_harga}', harga_api = '{$post_harga_api}', harga_premium = '{$post_harga_premium}', harga_h2h = '{$post_harga_h2h}', tipe = '{$post_tipe}', status = '{$post_status}', provider = '{$post_provider}' WHERE id = '{$post_pid}'");
	        if ($edit_layanan === true) {
				$_SESSION['alert'] = ['success', 'Success!', 'Layanan successfully change.'];
				exit(header("Location: ".base_url('/admin/trxpulsa/layanan')));
			        } 
			}
	} 
...
if (isset($_POST['addlayanan'])) {
	...
	$_SESSION['alert'] = ['danger', 'Failed!', 'Masih ada form kosong.'];
			...
			$insert_srvc = $db->query("INSERT INTO layanan_pulsa VALUES('', '', '{$post_code}', '{$post_kategori}', '{$post_layanan}', '{$post_harga_web}', '{$post_harga_api}', '{$post_harga_premium}', '{$post_harga_h2h}', '{$post_profit}', 'Normal', '{$post_provider}', '{$post_tipe}', '{$post_note}')");
			if ($insert_srvc === true) {
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="javascript:;" onclick="modal('Add Layanan','<?= base_url(); ?>admin/trxpulsa/service_data/add-layanan','','md')">
...
$(document).ready(function() {
    $('#datatabl3s').DataTable({
        ...
        "render" : function(data, type, row) {
            var btn = "<a href = \"javascript:;\" onclick=\"modal('Detail Layanan','<?= base_url(); ?>/admin/trxpulsa/service_data/detail_layanan?id="+data+"')\"><p class=\"font-medium-5 mr-1\" style=\"text-decoration:none;\"><i class=\"mdi mdi-eye mdi-24px\"></i></a></p><a href = \"javascript:;\" onclick=\"modal('Edit Layanan','<?= base_url(); ?>/admin/trxpulsa/service_data/edit_layanan?id="+data+"')\"><p class=\"font-medium-5 ml-1\" style=\"text-decoration:none;\"><i class=\"mdi mdi-pencil-box-outline mdi-24px\"></i></a></p><a href = \"javascript:;\" onclick=\"modal('Delete Layanan','<?= base_url(); ?>/admin/trxpulsa/service_data/delete_layanan?id="+data+"')\"><p class=\"font-medium-5 mr-1\" style=\"text-decoration:none;\"><i class=\"mdi mdi-trash-can mdi-24px\"></i></a></p>"
            return btn;
        }
        } 
                     ]
