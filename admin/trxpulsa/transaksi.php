	...
	if (!$post_oid) {
		...
		$edit_trxx = $db->query("UPDATE pembelian_pulsa SET keterangan = '{$post_nb}', status = '{$post_status}', tanggal_at = '{$dt_update}' WHERE oid = '{$post_oid}'"); 
			if ($edit_trxx === true) {
				...
				exit(header("Location: ".base_url('/admin/trxpulsa/transaksi')));
			} else {
...
$(document).ready(function() {
    $('#datatabl3s').DataTable({
        ...
        "ajax": "<?= base_url(); ?>/admin/trxpulsa/data_ajax/admin_pesanan_pulsa.php",
        columnDefs : [
        {
        "searhable" : false,
        "orderable" : false,
        ...
        "render" : function(data, type, row) {
            var btn = "<a href = \"javascript:;\" onclick=\"modal('Detail Pesanan','<?= base_url(); ?>/admin/trxpulsa/data_ajax/view-trxpulsa?id="+data+"')\"><p class=\"font-medium-5 mr-1\" style=\"text-decoration:none;\"><i class=\"mdi mdi-eye mdi-24px\"></i></a></p><a href = \"javascript:;\" onclick=\"modal('Edit Pesanan','<?= base_url(); ?>/admin/trxpulsa/data_ajax/edit-trxpulsa?id="+data+"')\"><p class=\"font-medium-5 ml-1\" style=\"text-decoration:none;\"><i class=\"mdi mdi-pencil-box-outline mdi-24px\"></i></a></p>"
            return btn;
        }
        } 
                     ]
