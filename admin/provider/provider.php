...
if (isset($_POST['edit'])) {
	...
	$post_merchant_id = $db->real_escape_string(e(@$_POST['merchant_id']));
	if (!$post_code) {
		...
		$edit_prov = $db->query("UPDATE provider SET link = '{$post_link}', api_key = '{$post_api_key}', api_id = '{$post_api_id}' WHERE code = '{$post_code}'"); 
			if ($edit_prov === true) {
