...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) AND strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    $data = [];
    $query = mysqli_query($db, "SELECT * FROM pembelian_pulsa WHERE user = '$session' AND tipe = 'Pulsa' ORDER BY id DESC");
    while($fetch = mysqli_fetch_assoc($query)) {
        ...
        $data[] = [
            ...
            '<div class="input-group">
                ...
                <button onclick="salin('.$txt_salin.');" class="btn btn-warning btn-sm" style="border-top-left-radius: 0;border-bottom-left-radius: 0;"><i class="fa fa-copy"></i></button>
            </div>',
